...
```
Pro tip: note how we're using `==` in the filter function. This is how we match things in R when making comparisons. (OK, there's more to it than that, but you get the picture here.)


```
library(ggplot2)
ggplot(movements_aus)+
...
```{r, fig.show = 'hold'}
ozflights::international_freight()
```
